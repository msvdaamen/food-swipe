...
func TestGetUserByEmail(t *testing.T) {
	...
	t.Run("Get user by email", func(t *testing.T) {
		...
		getUserByEmail(t, pool)
	})
}

func getUserByEmail(t *testing.T, dbPool *pgxpool.Pool) {
	assert := assert.New(t)
	...
	t.Run("should return user when found", func(t *testing.T) {
		user, err := adapter.GetUserByEmail(ctx, "test@test.com")
		assert.NoError(err)
		assert.Equal(user, createdUser)
	})

	t.Run("should return not found when no result", func(t *testing.T) {
		user, err := adapter.GetUserByEmail(ctx, "test2@test.com")
