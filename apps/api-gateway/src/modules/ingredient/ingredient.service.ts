...
export class IngredientService extends DbService {
  async all(
  ...
  ): Promise<PaginatedData<IngredientEntity>> {
    const wheres = [];
    if (payload.search) {
    ...
    const result = await this.database
      ...
      .from(ingredients)
      .where(and(...wheres))
      .orderBy(sortFunction, ingredients.id)
      .limit(payload.limit)
    ...
    return {
      data: result,
      pagination: CreatePagination(total, payload.limit, payload.page),
    };
  }

  async findByName(name: string): Promise<IngredientEntity | null> {
    const [ingredient] = await this.database
      .select()
      .from(ingredients)
